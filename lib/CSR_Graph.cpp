...
/**
 * Check wether or not two CSR Graphs are equal, regardless of edge order
 *
 **/
bool CSR_Graph::equals(CSR_Graph *g){
	if(nnodes != (g->nnodes))
	{
		printf("Number of nodes differs!\n");
		//return false;
	}
	
	if(nedges != (g->nedges))
	{
		printf("Number of edges differs!\n");
		//return false;
	}
	
	if(getTotalWeight() != g->getTotalWeight())
	{
		printf("Total weight differs!\n");
		//return false;
	}
	...
	for(i = 0; i < nnodes; i++)
	{
		...
		if(getOutDegree(i) != g->getOutDegree(i))
		{
			printf("Outdegree of %d differs (%d %d)\n", i, getOutDegree(i), g->getOutDegree(i));
			//return false;
		}
		
		for(j = 0; j < getOutDegree(i); j++)
		{
			edge = g->findDestination(i, getDestination(i, j), getWeight(i, j));
			if(!edge)
			{
				printf("No edge found from %d to %d\n", i, getDestination(i, j));
				//return false;
			}
		}
	}
...
/**
 * Prints the contents of the CSR Graph
 *
 **/
void CSR_Graph::toString(){
	unsigned i, j;
	...
	for(i = 0; i < nnodes; i++)
	{
		for(j = 0; j < getOutDegree(i); j++)
		{
			printf("CSR: node %d to %d with cost %d\n", i, getDestination(i,j), getWeight(i, j));
		}
	}
	
	for(i = 1; i <= nedges; i++)
	{
		printf("CSR: edge %d to %d with cost %d\n", i, edgessrcdst[i], edgessrcwt[i]);
	}
	
	for(i = 0; i < nnodes; i++)
	{
		printf("CSR: first edge of %d is %d and outdegree is %d\n", i, psrc[i], outdegree[i]);
	}
